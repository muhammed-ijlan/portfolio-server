...
const { newsCategoryService, newsPostService } = require("../services/_index");
...
exports.addCategory = async (req, res, next) => {
  try {
    ...
    await newsCategoryService.createNewCategory(reqBody);
...
exports.getCategories = async (req, res, next) => {
  try {
    ...
    const categories = await newsCategoryService.getCategories(options);
...
exports.changeStatus = async (req, res, next) => {
  try {
    ...
    await newsCategoryService.updateStatus(id, blockedStatus);
    await newsPostService.updateStatus(id, blockedStatus);
...
exports.editCategoty = async (req, res, next) => {
  try {
    ...
    let reqBody = req.body;
    let id = req.body.id;
    const category = await newsCategoryService.getOneCategory(id);
    if (reqBody.name) {
...
exports.userGetCategories = async (req, res, next) => {
  try {
    ...
    const categories = await newsCategoryService.userGetCategories(options);
    const response = new ResponseBody("Successfully retrieved Category", false, {
      maxRecords: categories.length ? categories[0].maxRecords : 0,
      categories: categories.length ? categories[0].data : [],
    });
